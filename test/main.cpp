...
TEST_CASE("Parse an empty CSV" * test_suite("Reader")) {
  ...
  auto shape = csv.shape();
  REQUIRE(shape.first == 0);
  REQUIRE(shape.second == 0);
}
...
TEST_CASE("Parse the most basic of CSV buffers" * test_suite("Reader")) {
  ...
  for (auto row: csv)
    for (auto cell: row)
      ...
      // REQUIRE(cell.value() == expected_cells[i++]);
  REQUIRE(i == expected_cells.size());
  auto shape = csv.shape();
  REQUIRE(shape.first == 3);
  REQUIRE(shape.second == 3);
}
